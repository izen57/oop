...
int read_object_file(object_t &object, struct Load lf)
{
    int err = OK;
    err = check_file(lf.file_name);
    if (err == OK)
    {
        FILE *f = open_file(lf.file_name);
        if (!f)
            err = FILE_INCORRECT;
        else
        {
...
int check_lines(object_t &obj)
{
    int error = OK;
    for (int i = 0; i < obj.object_points.count_points && error == OK; i++)
        error = check_points_in_line(obj.object_lines.array[i], obj.object_points.count_points);
    return error;
}
...
int load_object(object_t &object, struct Load lf)
{
    ...
    err = read_object_file(temp_object, lf);
    if (err == OK)
    {
        err = check_lines(temp_object);
        if (err == OK)
        {
            free_object(object);
